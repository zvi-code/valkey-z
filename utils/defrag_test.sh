...
get_info() {
  ...
  fragmentation_ratio=$(echo "$memory_info" | awk -F':' '/mem_fragmentation_ratio:/ {print $2}' | tr -d '[:space:]')
...
launch_server() {
  ...
  if [ -n "$SERVER" ]; then
    nohup ./$SERVER --io-threads 10 --activedefrag yes --maxmemory ${MIN_USED_MEMORY} --maxmemory-policy allkeys-lru --save ''&
#  else
...
#./src/valkey-benchmark -r 1000000 -n ${NUM_REQUESTS} -c 1 -q \
#        set key__rand_int__ xxxxxxx__rand_int__ EX 30

launch_loader() {
  echo "Line number: ${LINENO}"
  kill_loader
  nohup ./src/valkey-benchmark -c 80 -r 2000000000 -t set -d 24 -q -l&
  nohup ./src/valkey-benchmark -c 80 -r 50000000 -t set -d 1234 -q -l -P 3&
  nohup ./src/valkey-benchmark -c 80 -r 20000000 -t set -d 234 -q -l&
  nohup ./src/valkey-benchmark -c 80 -r 2000000000 -t set -d 24 -q -l&
  nohup ./src/valkey-benchmark -c 80 -r 10000000 -t set -d 12 -q -l -P 3&
  nohup ./src/valkey-benchmark -c 80 -r 25000000 -t set -d 123 -q -l -P 2&
...
start_defrag_wait_target_value() {
  ...
  if [[ "$fragmentation_ratio" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
    ...
    echo "$after_memory_info" | grep "mem_fragmentation_ratio\|used_memory_human\|defrag_time\|active_defrag\|expire\|evict\|latency_percentiles_usec_set\|cmdstat_set\|db0:keys=\|ncalls_util" >> "$filename"
    echo "$after_memory_info" | grep "mem_fragmentation_ratio\|used_memory_human\|defrag_time\|active_defrag\|expire\|evict\|latency_percentiles_usec_set\|cmdstat_set\|db0:keys=\|ncalls_util"
  fi
}
